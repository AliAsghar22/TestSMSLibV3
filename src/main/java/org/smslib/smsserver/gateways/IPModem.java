...
public class IPModem extends AGateway
{
	...
	public void create() throws Exception
	{
		...
		((ModemGateway) getGateway()).setCustomInitString(getProperties().getProperty(propName + "init_string", ""));
		if (getProperties().getProperty(propName + "smsc_number", "").length() > 0) ((ModemGateway) getGateway()).setSmscNumber(getProperties().getProperty(propName + "smsc_number"));
	}
}
